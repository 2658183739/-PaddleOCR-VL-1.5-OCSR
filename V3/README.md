...
接入真实模型时设置 `V3_MODEL_DIR`、`PYTHON_BIN` 和 `V3_DEVICE`；详细 API、运行边界与测试方法见 [`agent_demo/README.md`](agent_demo/README.md)。
...
决赛答辩优先使用 [`PaddleOCR_VL_OCSR_V3_template_final.pptx`](presentation/PaddleOCR_VL_OCSR_V3_template_final.pptx)；该版本逐页继承官方提供的 18 页视觉模板，并用冻结证据、真实 MolTrace 截图和可编辑图表替换原示例内容。全稿总览见 [`PaddleOCR_VL_OCSR_V3_template_final_overview.png`](presentation/PaddleOCR_VL_OCSR_V3_template_final_overview.png)。
